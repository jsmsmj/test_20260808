...
- **犬出現時**: 犬は画面の左右端に達すると反対側へラップする(`updateDogs()`、`dog.worldX`が`-half`または`BASE_W+half`を超えたら反転)。**このラップの瞬間、かつその犬が縦方向にも画面内(見えている状態)の時だけ**`playDogSE()`を呼ぶ(2026-08-08 最終版)。「画面の横から犬が出てきた時に鳴らす」という要望どおり、画面外(縦位置がまだ見えていない)でのラップでは鳴らない。`SE_DOG_SRCS`(=`seDogAudios`)の中から`Math.random()`で選ぶが、直前(`lastDogSEIndex`)と同じインデックスを引いた場合は`(idx + 1 + ランダムオフセット) % seDogAudios.length`で必ず別の音にずらす(連続再生の防止。件数を8→2に変えても壊れないよう`seDogAudios.length`基準で計算している)
  ...
  - **iPhone実機での処理落ち切り分けのため、`updateDogs()`内の`playDogSE()`呼び出しをコメントアウトして無効化している(2026-08-08)**。間引き(220ms)後もまだ重いようであれば、犬が画面に出入りするたびに走る「ラップ判定+`screenY`計算+SE再生」処理自体(オーディオの`cloneNode()`/`play()`呼び出しを含む)が原因の可能性がある。コメントアウトしても処理落ちが改善しなければ、SEではなく描画側(`drawImage`呼び出し数や影の描画、あるいは`checkCollision()`の犬ループ)が原因である可能性が高い。原因でないと判明すれば`if (...) playDogSE();`の行を元に戻してよい
