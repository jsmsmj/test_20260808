...
- **犬出現時**: 犬は画面の左右端に達すると反対側へラップする(`updateDogs()`、`dog.worldX`が`-half`または`BASE_W+half`を超えたら反転)。**このラップの瞬間、かつその犬が縦方向にも画面内(見えている状態)の時だけ**`playDogSE()`を呼ぶ(2026-08-08 最終版)。「画面の横から犬が出てきた時に鳴らす」という要望どおり、画面外(縦位置がまだ見えていない)でのラップでは鳴らない。`SE_DOG_SRCS`(=`seDogAudios`)の中から`Math.random()`で選ぶが、直前(`lastDogSEIndex`)と同じインデックスを引いた場合は`(idx + 1 + ランダムオフセット) % seDogAudios.length`で必ず別の音にずらす(連続再生の防止。件数を8→2に変えても壊れないよう`seDogAudios.length`基準で計算している)
  ...
  - **iPhone実機での処理落ちの原因と確定(2026-08-08)**: `updateDogs()`内の`playDogSE()`呼び出し(`if (screenY > -dogFrameH && screenY < BASE_H + dogFrameH) playDogSE();`)をコメントアウトしたところ実機の処理落ちが解消したため、犬のSE再生(`cloneNode()`+`play()`を伴う)が原因と確定した。匹数が多い場面(`MAX_DOGS=50`付近)では間引き(`DOG_SE_MIN_INTERVAL_MS=220ms`)後もなお、犬が画面端でラップするたびに`cloneNode()`で新しい`Audio`要素を生成して`play()`するコスト自体がiPhone実機では無視できない負荷になっていたとみられる。**現在この呼び出しはコメントアウトしたまま(犬のSEは鳴らない)**。将来的に有効化する場合は、`cloneNode()`を使い回すオーディオプール方式に変える、同時再生数の上限を設ける、等の対策と併用すること
...
「iPhone実機でコインSEが鳴らないことがある」原因切り分けのための一時的なデバッグ表示。Web Inspector(Macが無いと使えない)に頼らず、実機の画面上だけで原因を確認できるようにする目的で追加した。

- `playSE(baseAudio)`内、`node.play()`の結果を`.then()`/`.catch()`両方で拾い、`logSEDebug()`で画面左上の`#se-debug`に直近8件のログを表示する(`OK coin.mp3`のように成功、または`NG coin.mp3: NotAllowedError ...`のように失敗理由付きで表示)
- 失敗時のエラー名(`err.name`)が手がかりになる。例えば`NotAllowedError`なら自動再生ポリシー起因、`AbortError`ならまだ再生準備中に別の操作で中断された、など原因の切り分けに使える
- **`cloneNode()`自体が原因かどうかの見立て**: `cloneNode()`は`src`を含む属性はコピーするが、デコード済みバッファ等の内部状態はコピーされないため、クローンのたびに(キャッシュされたファイルからとはいえ)再デコードが走る。iOS Safariは同時に保持できるオーディオデコードのリソースに制限があるとされ、短時間に大量の`cloneNode()`+`play()`(磁石で複数枚まとめて回収、風船ばらまき後など)が重なると、そのリソース上限に達して一部が無音のまま失敗する可能性がある。`#se-debug`のログで`NG`が集中するタイミングが「コインを連続で取った直後」などに偏っていれば、この仮説を裏付けられる
- Playwrightでは`playCoinSE()`を直接呼び出し、`#se-debug`に`"0.5s OK coin.mp3"`のような行が追加されること、8件を超えると古い行から消えて常に最大8行に収まることを確認済み。ただしheadless Chromiumは実際の音声デコード制限を再現しないため、この機能自体の動作確認はできてもiPhone特有の失敗そのものは再現できない(**実機でログを見てもらう必要がある**)
- 原因切り分けが終わったら、`playSE()`内の`.then()`/`.catch()`ログ呼び出しと`#se-debug`のHTML/CSSを削除してよい
