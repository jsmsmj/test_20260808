...
### 横向きで開始した後、縦→横に戻すとゲーム画面が縮んだままになるバグ(2026-08-08 修正)

iPhoneで最初から横向きで開始した場合はゲーム画面のサイズが正しいのに、**一度縦持ちにしてからまた横向きに戻すと、ゲーム画面が(正しいサイズより)小さいまま**になる不具合が実機で報告された。

- 原因はiOS Safari特有の挙動で、画面回転の直後は`window.innerWidth`/`innerHeight`が新しいレイアウト確定前の値を一瞬返すことがある。`resize`イベントは回転のたびに発火するが、その1回だけで`applyViewportScale()`の計算を確定させていたため、確定前の(小さい)値を掴んだまま再計算されずに終わってしまうケースがあったとみられる(Playwrightは`setViewportSize`で即座に最終値になるため、この過渡的な挙動は再現できず実機でのみ発覚した)
...
- `#intro-overlay`(`#rotate-overlay`と同様、`#game`の**外側**・`#viewport`直下に配置。`position:fixed;inset:0;z-index:90`)を、ページ読み込み直後からデフォルトで表示している(HTML側で最初から`display:flex`。非表示にする時だけJSで`.hide`クラスを付与する方式)。目的(奥へ進んでコインを集める)・自機(自機の見た目を紹介する行。実際の`#player`と同じ配色`#ffca28`塗り/`#6d4c00`縁の丸を`.intro-player-dot`として表示)・操作方法(仮想ジョイパッド/WASD)・犬(ぶつかると愛情表現でライフが減る、ライフ0でゲームオーバー)・コイン(集める)・風船(お助けアイテム)を簡潔に説明する(2026-08-08: 当初「距離を稼ごう」だったが、コイン収集を主目的として伝えたいという要望で文言変更。また自機の紹介行が無かったので追加)
...
- `let introActive = true`が真の間、`checkCollision()`は先頭で即`return`(犬に当たらない)、プレイヤー移動も`if (!frozen && !introActive)`で止めている。ただし`gamePaused`/`orientationBlocked`のようなループ全体の停止はしていない(犬やコインは説明画面の背後で動き続ける。オーバーレイの半透明背景越しに見えるのは意図した演出)
...
「iPhone実機でコインSEが鳴らないことがある」原因切り分けのための一時的なデバッグ表示。Web Inspector(Macが無いと使えない)に頼らず、実機の画面上だけで原因を確認できるようにする目的で追加した。この表示のおかげで実機ログから`NotAllowedError`(HTMLAudioElement版)、続いて`suspended`状態での無音(自前Web Audio API版)が確認でき、最終的にHowler.jsへの切り替え(上記「効果音(SE)」節)につながった。

- `createSEHowl()`内、Howlの`'play'`/`'playerror'`イベントで`logSEDebug()`が画面左上の`#se-debug`に直近8件のログを表示する(`OK coin.mp3`=実際に再生開始できた、`NG coin.mp3: ...`=Howlerが報告した失敗理由)。Web Audio API版のような「`audioCtx.state`から推測する」のではなく、Howlerが実際に把握している成否をそのまま表示できるようになった
- Playwrightでは`playCoinSE()`を直接呼び出し、`#se-debug`に`"0.9s OK coin.mp3"`のような行が追加されること、8件を超えると古い行から消えて常に最大8行に収まること、8回連続再生してもすべて`OK`になることを確認済み
- Howler.jsで問題が解消したことが実機で確認でき次第、`createSEHowl()`内の`'play'`/`'playerror'`ログ呼び出しと`#se-debug`のHTML/CSSは削除してよい
